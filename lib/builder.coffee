...
class Builder
  ...
  build: (source, options, cb) ->
    ...
    @storage.generate_put_url "slug/#{@id}.#{ext}", (err, slug_put_url) =>
      @storage.generate_put_url "exit/#{@id}", (err, exit_put_url) =>
        @storage.generate_put_url "proc/#{@id}", (err, proc_put_url) =>
  ...
  build_request: (req, res, logger) ->
    ...
    require("builder").init().build req.body.source, options, (build, builder) ->
      ...
      res.writeHead 200
        ...
        "X-Env-Url":         builder.env_url
        "X-Proc-Url":        builder.proc_url
        "X-Addons-Url":      builder.addons_url
  ...
  stash_env_manifest: (env_manifest) ->
    url = "#{process.env.ANVIL_HOST}/env/#{@id}"
    @storage.create(url,env_manifest)
  ...
  env_url: (env_manifest) ->
    "#{process.env.ANVIL_HOST}/env/#{@id}"
  
  proc_url: (env_manifest) ->
    "#{process.env.ANVIL_HOST}/proc/#{@id}"
 
  addons_url: (env_manifest) ->
    "#{process.env.ANVIL_HOST}/addons/#{@id}"
